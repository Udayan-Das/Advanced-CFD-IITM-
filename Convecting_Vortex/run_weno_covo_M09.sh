#!/bin/bash
# ================================================================
# run_weno_covo_M09.sh — WENO Validation via COVO at Mach 0.9
# ================================================================
#
# PURPOSE:
#   Same as run_weno_covo.sh but at Mach 0.9 (high subsonic).
#   Prof said: "WENO would be dissipating things there but if your
#   code is right it would not explode."
#
#   At M=0.9 the sound speed is ~1.11 (vs 10 at M=0.1), so the
#   Lax-Friedrichs alpha is much smaller and the CFL is more relaxed.
#   WENO dissipation will smear the vortex more than at M=0.1 but
#   the solver must NOT blow up or produce NaN.
#
#   Results go into weno_covo_runs_M09/ (separate from the M=0.1 runs).
#   Post-process with plot_weno_covo_M09.ipynb.
#
# FOLDER STRUCTURE:
#   weno_covo_runs/
#   ├── C6_N041/
...
# Physics (inviscid COVO at M=0.9 — high subsonic stress test)
...
echo ""
echo -e "${BLD}╔══════════════════════════════════════════════╗${RST}"
echo -e "${BLD}║  WENO COVO VALIDATION (M=0.9) — CompSquare   ║${RST}"
echo -e "${BLD}╚══════════════════════════════════════════════╝${RST}"
echo ""
...
echo ""
echo -e "${BLD}╔══════════════════════════════════════════════════════════╗${RST}"
echo -e "${BLD}║           WENO COVO (M=0.9) — RESULTS SUMMARY          ║${RST}"
echo -e "${BLD}╚══════════════════════════════════════════════════════════╝${RST}"
echo ""
...
echo -e "  Next: open ${BLD}plot_weno_covo_M09.ipynb${RST} and run all cells."
echo ""
